...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      { source: '/(.*)', headers: globalHeaders },
      {
        source: '/api/:path*',
        headers: [
          { key: 'Access-Control-Allow-Origin', value: APP_URL || 'http://localhost:3000' },
  ...
  async redirects() {
    return [
      { source: '/shop', destination: '/store/shop', permanent: true },
      { source: '/shop/:path*', destination: '/store/shop/:path*', permanent: true },
