...
interface CartStore {
  ...
  updateQty:    (productId: string, quantity: number, selectedColor?: string) => void; // V011: P1-01
  clearCart:    () => void;
  // Computed
  count:        () => number;
  subtotal:     () => number;
  ...
  persist(
    (set, get) => ({
      ...
      removeItem: (productId, selectedColor) =>
        set(state => ({
          items: state.items.filter(i =>
            // If color is specified, only remove the exact color variant
            // If no color specified, remove all variants of this product
      ...
      updateQty: (productId, quantity, selectedColor) => {
        if (quantity < 1) { get().removeItem(productId, selectedColor); return; }
        set(state => ({
          // V011: P1-01 — scope to the exact (productId, selectedColor) tuple
          // so updating one colour variant does not mutate the others.
          items: state.items.map(i =>
            i.productId === productId && i.selectedColor === selectedColor
...
interface WishlistStore {
  ids:      string[];
  synced:   boolean;           // V025: true once server sync has run
  toggle:   (productId: string) => void;
  has:      (productId: string) => boolean;
  clear:    () => void;
  setSynced:(v: boolean) => void;
  setIds:   (ids: string[]) => void; // V025: merge server IDs
}
...
export const useWishlistStore = create<WishlistStore>()(
  persist(
    (set, get) => ({
      ...
      clear:  () => set({ ids: [], synced: false }),
      setSynced: (v) => set({ synced: v }),
      setIds: (incoming) => set(state => ({
        // Merge without duplicates
        ids:    [...new Set([...state.ids, ...incoming])],
        synced: true,
      })),
    }),
