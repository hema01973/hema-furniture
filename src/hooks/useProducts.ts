...
import type { IProduct, IProductFilters, ProductSortKey, ApiResponse, DashboardStats } from '@/types';

export function useAnalytics(options?: SWRConfiguration) {
  return useSWR<ApiResponse<DashboardStats>>('/api/v1/analytics', fetcher, {
...
export function useProductsInfinite(filters?: IProductFilters, sort?: ProductSortKey) {
  return useSWRInfinite<ApiResponse<{ products: IProduct[]; pagination: { pages: number } }>>(
    (pageIndex, prev) => {
      ...
      return `/api/v1/products${buildQuery({ ...filters, sort, page: pageIndex + 1, limit: 12 })}`;
    },
    fetcher,
...
export function useProduct(idOrSlug?: string, options?: SWRConfiguration) {
  return useSWR<ApiResponse<IProduct>>(
    idOrSlug ? `/api/v1/products/${idOrSlug}` : null,
    fetcher,
    { revalidateOnFocus: false, ...options }
  );
}
...
export function useProductSearch(query: string, enabled = true) {
  return useSWR<ApiResponse<{ products: IProduct[] }>>(
    enabled && query.length >= 2 ? `/api/v1/products?q=${encodeURIComponent(query)}&limit=8` : null,
    fetcher,
