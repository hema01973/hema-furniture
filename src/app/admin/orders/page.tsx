...
const STATUS_OPTIONS: OrderStatus[] = ['pending','confirmed','processing','shipped','out_for_delivery','delivered','cancelled'];
...
export default function AdminOrders() {
  ...
  const { data, mutate } = useSWR<{ success: boolean; data: { orders: IOrder[]; pagination: { total: number; pages: number } } }>(
    `/api/v1/orders?page=${page}&limit=20${statusF !== 'all' ? `&status=${statusF}` : ''}`,
    fetcher
  );
  ...
  const updateStatus = async (orderId: string, status: OrderStatus) => {
    const res  = await apiFetch(`/api/v1/orders/${orderId}`, { method: 'PUT', body: JSON.stringify({ status }) });
  ...
  const refundOrder = async (orderId: string) => {
    const res  = await apiFetch(`/api/v1/orders/${orderId}/refund`, { method: 'POST' });
    const json = await res.json();
    if (json.success) {
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                {detail.paymentStatus === 'paid' && (
                  <button onClick={() => refundOrder(detail._id)} className="mt-3 px-4 py-2 rounded-lg text-xs font-semibold bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 hover:bg-red-100 transition-colors">
                    Mark as Refunded
                  </button>
                )}
              </div>
