'use client';
// src/... — HemaV050: added Reviews nav, manager role support
import { useSession } from 'next-auth/react';
import { usePathname, useRouter } from 'next/navigation';
import { useEffect } from 'react';
import Link from 'next/link';
import { signOut } from 'next-auth/react';
// V036 FIX: import ADMIN_ROLES from single source of truth
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && !ADMIN_ROLES.has(session.user.role as string)) {
      router.replace('/');
    }
  ...
  if (status === 'unauthenticated' || !ADMIN_ROLES.has(session?.user.role as string)) {
