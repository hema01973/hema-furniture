...
export default function ProductDetailPage({ slug, initialProduct }: { slug: string; initialProduct?: IProduct }) {
  const { data, isLoading } = useSWR<{ success: boolean; data: IProduct }>(`/api/v1/products/${slug}`, fetcher);
  const p = data?.data ?? initialProduct;
