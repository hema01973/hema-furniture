      ...
      <div className="max-w-[1100px] mx-auto px-6 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
          ...
          <div className="bg-white dark:bg-[#1A1208] border border-[#E8DDD0] dark:border-[#2A1F14] rounded-2xl overflow-hidden">
            {items.map((item, idx) => (
              // V011: P1-01 — key must include selectedColor; the same product
              // can appear twice with different colours and would otherwise
              // collide on key (React) and break the variant updateQty path.
              <div key={`${item.productId}-${item.selectedColor ?? 'none'}`} className={`grid grid-cols-[90px_1fr_auto] gap-4 p-5 items-start ${idx < items.length-1 ? 'border-b border-[#E8DDD0] dark:border-[#2A1F14]' : ''}`}>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    {/* V011: P1-01 — thread item.selectedColor through every cart action. */}
                    <button onClick={() => updateQty(item.productId, item.quantity - 1, item.selectedColor)} className="w-8 h-8 rounded-lg border border-[#D0C4B4] dark:border-[#3A2D20] flex items-center justify-center text-base hover:bg-[#F2EDE6] dark:hover:bg-white/5 transition-colors">−</button>
                    <span className="w-8 text-center text-sm font-semibold text-[#1A1208] dark:text-[#F0EBE2]">{item.quantity}</span>
                    <button onClick={() => updateQty(item.productId, item.quantity + 1, item.selectedColor)} className="w-8 h-8 rounded-lg border border-[#D0C4B4] dark:border-[#3A2D20] flex items-center justify-center text-base hover:bg-[#F2EDE6] dark:hover:bg-white/5 transition-colors">+</button>
                  </div>
                  <button onClick={() => removeItem(item.productId, item.selectedColor)} className="text-xs text-gray-400 hover:text-red-500 transition-colors mt-2 underline">Remove</button>
                </div>
